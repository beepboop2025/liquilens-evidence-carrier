...
def test_gateway_container_is_attested_multiarch_distribution_not_authority():
    ...
    assert 'org.opencontainers.image.version="0.2.2"' in dockerfile
...
def test_gateway_only_release_tag_is_signed_main_bound_and_immutable():
    ...
    assert 'EXPECTED_GATEWAY_VERSION: "0.2.2"' in workflow
    ...
    assert 'assert distribution.version == "0.2.2"' in test_step
